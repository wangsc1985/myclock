...
import com.wang17.myclock.utils.CalendarHelper;
...
public class GanZhi {
    ...
    private static final DateTime MINIMAL_DATE = new DateTime(1999, 1, 1, 1, 0, 0);
    private static DateTime MAXIMAL_DATE = new DateTime(2050, 12, 31, 22, 59, 59);

    private Map<DateTime, SolarTerm> solarTermMap;
    ...
    public static DateTime getMaximalDate() {
        return MAXIMAL_DATE;
    }

    public static DateTime getMinimalDate() {
        return MINIMAL_DATE;
    }
     ...
     */
    public GanZhi(DateTime dateTime, TreeMap<DateTime, SolarTerm> solarTermTreeMap) throws Exception {
    ...
    private void convertToGanZhi(DateTime dateTime) throws Exception {
        if (dateTime.compareTo(MINIMAL_DATE) == -1 || dateTime.compareTo(MAXIMAL_DATE) == 1) {
            throw new Exception("当前时间：【"+dateTime.toShortDateString()+ "】超出时间允许范围【"+
        ...
        DateTime cal =new DateTime(2015,1,1);
        DateTime today = dateTime.getDate();
        int years = today.getYear() - 1955;

        for (Map.Entry<DateTime, SolarTerm> entry : this.solarTermMap.entrySet()) {
        ...
        // 月干支
        Map.Entry<DateTime, SolarTerm> nextSolarTerm = null;
        for (Map.Entry<DateTime, SolarTerm> entry : this.solarTermMap.entrySet()) {
            cal.set(entry.getKey().getYear(), entry.getKey().getMonth(), entry.getKey().getDay(), 0, 0, 0);
        ...
        DateTime normDateTime = new DateTime(1800, 0, 1, 0, 0, 0).addHours(-1);
